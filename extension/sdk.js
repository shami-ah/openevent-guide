var OpenEventGuide=(function(I){"use strict";const J="oe-guide-overlay",_="oe-guide-subtitle",C="oe-guide-highlight",U="oe-guide-spotlight";let w=null,p=null,d=null,g=null,$=null;function ce(){if(document.getElementById("oe-guide-overlay-styles"))return;const e=document.createElement("style");e.id="oe-guide-overlay-styles",e.textContent=`
  ...
  `,document.head.appendChild(e)}function le(){w||(ce(),w=document.createElement("div"),w.id=J,document.body.appendChild(w),g=document.createElement("div"),g.id=U,document.body.appendChild(g),p=document.createElement("div"),p.id=_,document.body.appendChild(p),d=document.createElement("div"),d.id=C,document.body.appendChild(d))}function W(e,o=5e3){p&&($&&clearTimeout($),p.textContent=e,p.classList.add("visible"),o>0&&($=setTimeout(()=>{p==null||p.classList.remove("visible")},o)))}function A(){$&&clearTimeout($),p==null||p.classList.remove("visible")}function q(e,o=0){const n=document.querySelector(e);if(!n||!d)return console.warn(`[oe-guide] Element not found: ${e}`),()=>{};const i=n.getBoundingClientRect(),s=6;d.style.top=`${i.top-s}px`,d.style.left=`${i.left-s}px`,d.style.width=`${i.width+s*2}px`,d.style.height=`${i.height+s*2}px`,d.classList.add("visible"),g&&(g.style.clipPath=`polygon(
    ...
    /* ── Proactive hint ────────────────────────── */
    .oeg-hint{position:fixed;bottom:92px;right:24px;background:${v};color:${i};padding:12px 16px;border-radius:14px;box-shadow:0 4px 20px rgba(0,0,0,.15);font-size:14px;line-height:1.5;max-width:280px;animation:oeg-fadein .3s ease;cursor:pointer;border:1px solid ${r};z-index:99991}
    .oeg-hint::after{content:'';position:absolute;bottom:-6px;right:24px;width:12px;height:12px;background:${v};border-right:1px solid ${r};border-bottom:1px solid ${r};transform:rotate(45deg)}
    ...
    /* ── Picker (call or chat) ─────────────────── */
    .oeg-picker{position:fixed;bottom:92px;right:24px;background:${v};border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,.18);padding:8px;display:flex;flex-direction:column;gap:4px;min-width:200px;animation:oeg-fadein .2s ease;z-index:99992;border:1px solid ${r}}
